...
namespace OpenDentBusiness.WebTypes.Shared.XWeb {
	public class XWebs {
		...
		public static XWebResponse GetHpfUrlForPayment(long patNum,string payNote,bool isMobile,double amount,bool createAlias) {
			...
			return new XWebInputHpfPayment(patNum,isMobile,amount,payNote,createAlias).GenerateOutput();
		}
		...
		public static XWebResponse ReturnPayment(long patNum,string payNote,double amount,long creditCardNum) {
			//No need to check RemotingRole;no call to db.
			return new XWebInputDTGPaymentReturn(patNum,payNote,amount,creditCardNum).GenerateOutput();
		}
		...
		private class XWebInputHpfPayment:XWebInputHpf {
			...
			public XWebInputHpfPayment(long patNum,bool isMobile,double amount,string payNote,bool createAlias) : base(XWebTransactionType.CreditSaleTransaction,patNum,isMobile) {
				_amount=amount;
				_payNote=payNote;
		...
		private class XWebInputDTGPaymentSale:XWebInputDTG {
			...
			protected virtual bool InsertPaymentOnApproval { get { return true; } }
			...
			protected override void PostProcessOutput(XWebResponse response) {
				...
				if(InsertPaymentOnApproval) { //Insert Payment, PaySplit, and set FK.					
					response.PaymentNum=Payments.InsertFromXWeb(
						...
						response.GetFormattedNote(InsertPositivePayment),"");//todo: create a formatted receipt to show the web user after the payment has been accepted
				}
		...
		private class XWebInputDTGPaymentVoid:XWebInputDTG {
			...
			protected override void PostProcessOutput(XWebResponse response) {
				...
				response.PaymentNum=Payments.InsertFromXWeb(
					...
					response.GetFormattedNote(_insertPositivePayment),"");
			}
		}
		...
		private class XWebInputDTGPaymentReturn:XWebInputDTGPaymentSale {
			...
			protected override bool InsertPaymentOnApproval { get { return true; } }
			protected override bool InsertPositivePayment {	get {	return false; } }

			public XWebInputDTGPaymentReturn(long patNum,string payNote,double amount,long creditCardNum) : base(patNum,payNote,amount,creditCardNum,XWebTransactionType.CreditReturnTransaction) {
			}
		}
