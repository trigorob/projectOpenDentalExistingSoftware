...
namespace OpenDentBusiness {
	public partial class ConvertDatabases {
		public static System.Version LatestVersion=new Version("16.3.47.0");//This value must be changed when a new conversion is to be triggered.
		...
		private static void To16_3_20() {
			if(FromVersion<new Version("16.3.20.0")) {
				...
				string command;
				long claimReportReceiveInterval=PIn.Long(Db.GetScalar("SELECT ValueString FROM preference WHERE PrefName='ClaimReportReceiveInterval'"));
				if(claimReportReceiveInterval==5) {//Update to new default.
					command="UPDATE preference SET ValueString='30' WHERE PrefName='ClaimReportReceiveInterval'";
					Db.NonQ(command);
				}
				else if(claimReportReceiveInterval<5) {
		...
		private static void To16_3_47() {
			if(FromVersion<new Version("16.3.47.0")) {
				ODEvent.Fire(new ODEventArgs("ConvertDatabases","Upgrading database to version: 16.3.47"));//No translation in convert script.
				string command;
				...
				command="UPDATE preference SET ValueString = '16.3.47.0' WHERE PrefName = 'DataBaseVersion'";
				Db.NonQ(command);
			}
			//To16_3_X();
		}
