...
namespace OpenDentBusiness{
	...
	public class EhrMeasures{
		...
		public static List<EhrMeasure> SelectAllMu2(DateTime dateStart,DateTime dateEnd,long provNum) {
			...
			retVal.RemoveAll(x => 
				x.MeasureType==EhrMeasureType.DrugDrugInteractChecking ||
				x.MeasureType==EhrMeasureType.DrugFormularyChecking ||
				x.MeasureType==EhrMeasureType.ProtectElectHealthInfo ||
				x.MeasureType==EhrMeasureType.ImmunizationRegistries ||
				x.MeasureType==EhrMeasureType.SyndromicSurveillance ||
				x.MeasureType==EhrMeasureType.PatientList ||
				x.MeasureType==EhrMeasureType.ClinicalInterventionRules
			);
			Stopwatch s=new Stopwatch();
			for(int i=0;i<retVal.Count;i++) {
				s.Restart();
