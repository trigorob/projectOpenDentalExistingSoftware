...
namespace OpenDentBusiness {
	...
	public class Documents {
		...
		public static Bitmap GetThumbnail(Document doc,string patFolder,int size){
			...
			if(PrefC.AtoZfolderUsed==DataStorageType.LocalAtoZ && !File.Exists(fullName)) {//Check if file exists on dropbox here as well?
				return NoAvailablePhoto(size);
			}
		...
		public static bool DocExists(long docNum) {
			...
			if(PrefC.AtoZfolderUsed==DataStorageType.LocalAtoZ) {
				...
				return DropboxApi.FileExists(ProgramProperties.GetPropVal(Programs.GetProgramNum(ProgramName.Dropbox),DropboxApi.PropertyDescs.AccessToken)
					,ODFileUtils.CombinePaths(ImageStore.GetPatientFolder(patCur,ImageStore.GetPreferredAtoZpath()),docCur.FileName,'/'));
			}
