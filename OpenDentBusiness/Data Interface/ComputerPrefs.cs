...
namespace OpenDentBusiness {
	public class ComputerPrefs {
		...
		public static void UpdateLocalComputerOS() {
			...
			string enumAsString=POut.String(Environment.OSVersion.Platform.ToString());
			if(LocalComputer.ComputerOS.ToString()==enumAsString) {//We only want to update this column if there is indeed a difference between values.
				return;
			}
      ...
      string command="UPDATE computerpref SET ComputerOS = '"+enumAsString+"' "
				+"WHERE ComputerPrefNum = "+POut.Long(ComputerPrefs.LocalComputer.ComputerPrefNum);
      Db.NonQ(command);
