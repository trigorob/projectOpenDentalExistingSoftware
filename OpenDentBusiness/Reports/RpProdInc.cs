...
namespace OpenDentBusiness {
	public class RpProdInc {
      ...
      if(!hasClinicInfo) {
		...
		///<summary></summary>
		public static DataSet GetProviderPayrollDataForClinics(DateTime dateFrom,DateTime dateTo,List<Provider> listProvs,List<Clinic> listClinics,bool hasAllProvs,bool hasAllClinics,bool isDetailed,bool showUnearned) {
			if(listClinics.Count>0) {
				_hasClinics=true;
			}
			DataSet dataSet=GetProviderPayrollDataSet(dateFrom,dateTo,listProvs,listClinics,hasAllProvs,hasAllClinics,showUnearned);
			DataTable tableProduction=dataSet.Tables["tableProduction"];
		...
		public static DataSet GetDailyProdIncDataSet(DateTime dateFrom,DateTime dateTo,List<Provider> listProvs,List<Clinic> listClinics,bool writeOffPay,bool hasAllProvs,bool hasAllClinics,bool showUnearned) {
			...
			if(!showUnearned) {//no unearned, doesn't matter if providers are specified or not.
				wherePtIncomeProvs+=" AND paysplit.ProvNum != 0 ";
			}
			if(!hasAllClinics && listClinicNums.Count>0) {
		...
		public static DataSet GetProviderPayrollDataSet(DateTime dateFrom,DateTime dateTo,List<Provider> listProvs,List<Clinic> listClinics,bool hasAllProvs,bool hasAllClinics,bool showUnearned) {
			if(RemotingClient.RemotingRole==RemotingRole.ClientWeb) {
				return Meth.GetDS(MethodBase.GetCurrentMethod(),dateFrom,dateTo,listProvs,listClinics,hasAllProvs,hasAllClinics,showUnearned);
			}
			...
			if(!showUnearned) {
				whereProv+=" AND paysplit.ProvNum != 0 ";
			}
			if(hasClinics) {
				whereClin=" AND paysplit.ClinicNum IN ("+String.Join(",",listClinicNums)+") ";
			}
