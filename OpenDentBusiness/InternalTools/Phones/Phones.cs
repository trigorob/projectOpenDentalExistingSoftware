...
namespace OpenDentBusiness {
	...
	public class Phones {		
		...
		public static void SetProximity(bool isProximal,string computerName) {
			if(RemotingClient.RemotingRole==RemotingRole.ClientWeb) {
				Meth.GetVoid(MethodBase.GetCurrentMethod(),isProximal,computerName);
				return;
			}
			string command="UPDATE phone SET DTProximal = "+DbHelper.Now()+" IsProximal = "+POut.Bool(isProximal)
				+" WHERE Extension = (SELECT PhoneExt FROM phoneempdefault WHERE ComputerName='"+POut.String(computerName)+"')";
			Db.NonQ(command);
		}
