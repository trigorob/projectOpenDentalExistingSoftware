...
namespace OpenDentBusiness {
    public class KPIDowntime
    {
        ...
        public static DataTable GetDowntime(DateTime dateStart, DateTime dateEnd)
        {
            ...
            SELECT  appointment.ProvNum , sec_to_time((sum((CHAR_LENGTH(appointment.Pattern) - CHAR_LENGTH(REPLACE(appointment.Pattern, 'X', ''))) ))*avg(apptview.RowsPerIncr)*60)  AS DownTime
				FROM appointment 
                CROSS JOIN apptview         
                WHERE EXISTS (	SELECT *
								FROM procedurelog
