    ...
    private int _mouseClickCount=0;
			...
			using(Font cellFontNormal=new Font(tempFont,FontStyle.Regular)) {
				...
				for(int i=0;i<rows.Count;i++) {
					...
					if(wrapText) {
						...
						if(_rowHeights[i]<4) {
							_rowHeights[i]=(int)Math.Ceiling(g.MeasureString("Any",cellFont,100,_format).Height+1);
						}
					}
						...
						//if(rows[i].Height==0) {//not set
						//	RowHeights[i]=(int)Math.Ceiling(g.MeasureString("Any",cellFont,100).Height+1);
						//}
						//else {
					...
					if(noteW>0 && rows[i].Note!="") {
						if(rows[i].Note.Length<=_noteLengthLimit) {//Limiting the characters shown because large emails can cause OD to hang.
							NoteHeights[i]=(int)Math.Ceiling(g.MeasureString(rows[i].Note,cellFontNormal,noteW,_format).Height);//Notes cannot be bold.  Always normal font.
						}
						else {
							NoteHeights[i]=(int)Math.Ceiling(g.MeasureString(rows[i].Note.Substring(0,_noteLengthLimit),cellFontNormal,noteW,_format).Height);
						}
					}
					if(i==0) {
						RowLocs[i]=0;
      ...
      _mouseClickCount=0;
		...
		private void CreateEditBox() {
			...
			if(-vScroll.Value+1+titleHeight+headerHeight+RowLocs[selectedCell.Y]+_rowHeights[selectedCell.Y]+hScrollBarHeight
				>this.DisplayRectangle.Bottom) 
			{
				int onScreenPixels=vScroll.Value+DisplayRectangle.Height-titleHeight-headerHeight-(RowLocs[selectedCell.Y])-hScrollBarHeight;
			...
			else if(-vScroll.Value+1+titleHeight+headerHeight+RowLocs[selectedCell.Y]<this.DisplayRectangle.Top+titleHeight+headerHeight) {
