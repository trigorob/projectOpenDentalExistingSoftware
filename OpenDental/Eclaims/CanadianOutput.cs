...
namespace OpenDental.Eclaims {
	public class CanadianOutput {
			...
			InsSub insSub)
		{
			...
			try {
				result=Canadian.PassToIca(strb.ToString(),clearinghouseClin);
			}
			...
			do {
				...
				if(!exit && !isAutomatic) {
					try {
						...
						CodeBase.MsgBoxCopyPaste msgbox=new CodeBase.MsgBoxCopyPaste(Lan.g("CanadianOutput","Failed to display one of the ROT responses, here is the raw message")+": "+Environment.NewLine+result);
						msgbox.ShowDialog();
					}
				}
			} while(!exit);
