...
namespace OpenDental.UI {
	public class ComboBoxMulti : UserControl {
		...
		public void SetSelected(int index,bool setToValue){
			if(setToValue && !ArraySelectedIndices.Contains(index)) {
				gridMain.SetSelected(index,true);
			}
			else if(!setToValue && ArraySelectedIndices.Contains(index)){
				gridMain.SetSelected(index,false);
			}
			FillText();
		}
		...
		private void FillGrid() {
			...
			gridMain.Height=gridMain.RowHeights.Sum()+3;//1 (for upper grid border) +1 (for lower grid border) +1 (for toolbar magic space at top) = 3
