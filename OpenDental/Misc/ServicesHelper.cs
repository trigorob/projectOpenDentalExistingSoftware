...
namespace OpenDental {
	...
	internal class ServicesHelper {
		...
		public static bool TryInstallOpenDentalService(bool isSilent) {
			...
			try {
				List<ServiceController> listOpenDentalServices=GetServicesByExe("OpenDentalService.exe");
				...
				if(!Install("OpenDentalService",odServiceExeFI)) {
					return false;
				}
				...
				//Now that the service has finally installed we need to try and start it.
				listOpenDentalServices=GetServicesByExe("OpenDentalService.exe");
				if(listOpenDentalServices.Count<1) {
					return false;
				}
				if(!Start(listOpenDentalServices[0])) {
					return false;
				}
				return true;
			}
		...
		private static bool CreateConfigForEConnector(bool isListening) {
			...
			return CreateServiceConfigFile(eConnectorConfigPath);
		}
		...
		public static bool CreateConfigForOpenDentalService() {
			...
			return CreateServiceConfigFile(odServiceConfigPath);
		}
		...
		private static bool CreateServiceConfigFile(string filePath) {
