...
//		private System.Windows.Forms.PrintDialog printDialog2;
    ...
    private ArrayList ALPreAuth;
		...
		private void InitializeComponent(){
			...
			this.radioTreatPlanSortTooth.Location = new System.Drawing.Point(14, 43);
			...
			this.radioTreatPlanSortTooth.TabIndex = 56;
			this.radioTreatPlanSortTooth.Text = "Tooth";
			this.radioTreatPlanSortTooth.UseVisualStyleBackColor = true;
			this.radioTreatPlanSortTooth.CheckedChanged += new System.EventHandler(this.radioTreatPlanSort_CheckedChanged);
			// 
			...
			this.radioTreatPlanSortOrder.Location = new System.Drawing.Point(14, 58);
			...
			this.radioTreatPlanSortOrder.TabIndex = 55;
	...
	private List<ProcTP> LoadActiveTP(ref TreatPlan treatPlan) {
		...
		List<Procedure> listProcForTP=Procedures.GetListTPandTPi(Procedures.GetManyProc(listTreatPlanAttaches.Select(x=>x.ProcNum).ToList(),false)
			,listTreatPlanAttaches).ToList();
		...
		private List<Document> SaveTPAsDocument(bool isSigSave,Sheet sheet=null) {
			...
			foreach(long docCategory in categories) {//usually only one, but do allow them to be saved once per image category.
				...
				if(PrefC.AtoZfolderUsed==DataStorageType.LocalAtoZ) {
					...
					FormP.DisplayText="Uploading Treatment Plan to Dropbox...";
					...
					DropboxApi.TaskStateUpload state=DropboxApi.UploadAsync(ProgramProperties.GetPropVal(Programs.GetProgramNum(ProgramName.Dropbox),DropboxApi.PropertyDescs.AccessToken)
						,ImageStore.GetPatientFolder(PatCur,"")
						,fileName+".pdf"
				...
				docSave.FileName=fileName+".pdf";//file extension used fo rboth DB images and AtoZ images
