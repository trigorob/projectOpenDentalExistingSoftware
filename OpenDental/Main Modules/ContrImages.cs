...
namespace OpenDental {
	...
	public class ContrImages:System.Windows.Forms.UserControl {
		...
		///localPath will be set only if using DropBox and an image was imported.  We want to use the local version instead of re-downloading what was just uploaded.</summary>
		private void SelectTreeNode(TreeNode node,string localPath="") {
			...
			if(nodeId.NodeType==ImageNodeType.Category) {
				...
				if(PrefC.AtoZfolderUsed==DataStorageType.DropboxAtoZ) {
					actionCloseDownloadProgress=ODProgress.ShowProgressStatus("ImageDownload",Lan.g("ContrImages","Downloading from Dropbox")+"...");
				}
						...
						try {
							...
							if(PrefC.AtoZfolderUsed==DataStorageType.LocalAtoZ) {
								...
								if(localPath!="") {
									...
									FormP.DisplayText="Downloading EOB from Dropbox...";
						...
						try {
							...
							if(PrefC.AtoZfolderUsed==DataStorageType.LocalAtoZ) {
								...
								if(localPath!="") {
									...
									//Download PDF into temp directory for displaying.
									FormProgress FormP=new FormProgress();
									FormP.DisplayText="Downloading Document from Dropbox...";
									FormP.NumberFormat="F";
									FormP.NumberMultiplication=1;
									FormP.MaxVal=100;//Doesn't matter what this value is as long as it is greater than 0
									FormP.TickMS=1000;
						...
						try {
							...
							if(PrefC.AtoZfolderUsed==DataStorageType.LocalAtoZ) {
								...
								if(localPath!="") {
									...
									//Download PDF into temp directory for displaying.
									FormProgress FormP=new FormProgress();
									FormP.DisplayText="Downloading Amendment from Dropbox...";
									FormP.NumberFormat="F";
									FormP.NumberMultiplication=1;
									FormP.MaxVal=100;//Doesn't matter what this value is as long as it is greater than 0
									FormP.TickMS=1000;
		...
		private void ToolBarImport_Click() {
			...
			if(ClaimPaymentNum!=0) {//eob
				...
				if(PrefC.AtoZfolderUsed==DataStorageType.DropboxAtoZ) {
					actionCloseUploadProgress=ODProgress.ShowProgressStatus("ImageUpload",Lan.g("ContrImages","Uploading to Dropbox")+"...");
				}
