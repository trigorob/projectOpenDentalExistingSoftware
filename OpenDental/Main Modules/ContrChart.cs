...
namespace OpenDental {
	...
	public class ContrChart : System.Windows.Forms.UserControl	{
		...
		///<summary>Will only be set to true if trying to browse to eRx with an older browser (the first time the user tries).</summary>
		private bool _isErxSuspended=false;
...
//			IE.Navigate(newCropUrl,null,null,PostDataBytes,additionalHeaders);
			...
			#region Enforce Latest IE Version Available.
			int browserVersion;
			...
			#endregion Enforce Latest IE Version Available.
...
//todo: compare with: Bridges.ECW.AptNum, no need to generate PDF segment, pdfs only with eCW and this button not available with eCW integration
		...
		private bool AddProcHelper(Procedure ProcCur) {
			...
			long verifyCode;
			if(AutoCodeItems.ShouldPromptForCodeChange(ProcCur,procCodeCur,PatCur,0,listClaimProcs,out verifyCode)) {
