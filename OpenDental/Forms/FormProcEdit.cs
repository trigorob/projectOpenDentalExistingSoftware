...
///<summary></summary>
    ...
    "ou should reprint the claim if any significant changes are made.";
			...
			this.butNoneOrderProv.Autosize = true;
			...
			this.butNoneOrderProv.TabIndex = 286;
			...
			this.butPickOrderProvInternal.TabIndex = 285;
			...
			this.butPickOrderProvReferral.TabIndex = 301;
		...
		private void comboClinic_SelectedIndexChanged(object sender,EventArgs e) {
			...
			ProcCur.ClinicNum=_selectedClinicNum;//legacy code support. We should move all "saving" behavior to ok click
			ClaimProcsForProc.ForEach(x => x.ClinicNum=_selectedClinicNum);
		...
		private void comboProv_SelectedIndexChanged(object sender,EventArgs e) {
			...
			ProcCur.ProvNum=_selectedProvNum;//legacy code support. We should move all "saving" behavior to ok click
			ClaimProcsForProc.ForEach(x => x.ProvNum=_selectedProvNum);
		}
		...
		private void butPickProv_Click(object sender,EventArgs e) {
			...
			ProcCur.ProvNum=_selectedProvNum;//should this line be move to OK click?
			ClaimProcsForProc.ForEach(x => x.ProvNum=_selectedProvNum);
		...
		private void SetControlsEnabled(bool isSilent) {
			...
			bool hasDateLock = false;
			if(ProcCur.ProcStatus==ProcStat.C) {
				hasDateLock=Security.IsGlobalDateLock(Permissions.ProcComplEdit,ProcCur.DateEntryC,isSilent);//really only used to silence other security messages.
			}
			if(ProcCur.ProcStatus!=ProcStat.C) {
				return;
			}
      ...
      SetControlsUpperLeft();
		...
		private void butSetComplete_Click(object sender, System.EventArgs e) {
			...
			textNotes.Text+=ProcCodeNotes.GetNote(_selectedProvNum,ProcCur.CodeNum);
		...
		private bool EntriesAreValid(){
			...
			if(ProcCur.ProvNum==0){//this works because ProvNum gets set when the user changes the combobox. (Can use _selectedProvNum I think.)
			...
			if(ProcOld.ProcStatus!=ProcStat.C && ProcCur.ProcStatus==ProcStat.C){//if status was changed to complete
				...
				if(ProcOld.ProcStatus==ProcStat.C){//that was already complete
					...
					if(ProcOld.ProcDate!=PIn.Date(textDate.Text)) {//If user changed the procedure date
						//We need to check to make sure they did not backdate the procedure past the global lock date.
						if(!Security.IsAuthorized(Permissions.ProcComplEdit,PIn.Date(textDate.Text))) {
							//This puts out a message that isn't very helpful.
							return false;
						}
					}
				}
		...
		private void SaveAndClose(){
          ...
          string range="";
			...
			ClaimProcsForProc.ForEach(x => x.ProvNum=_selectedProvNum);//Where are these changes saved to the DB?
			ClaimProcsForProc.ForEach(x => x.ClinicNum=_selectedClinicNum);//Where are these changes saved to the DB?
			//Recall synch--------------------------------------------------------------------------------------------------------------------------------------
			...
			//Autocodes----------------------------------------------------------------------------------------------------------------------------------------
			long verifyCode;
			if(ProcOld.ProcStatus==ProcStat.C && !Security.IsAuthorized(Permissions.ProcComplEdit,ProcCur.DateEntryC,true)) {
				DialogResult=DialogResult.OK;
				return;//Don't check auto codes if the user is not able to edit completed procedures.
			}
			if(AutoCodeItems.ShouldPromptForCodeChange(ProcCur,ProcedureCode2,PatCur,listBoxTeeth.SelectedIndices.Count,ClaimProcsForProc,out verifyCode)) {
		...
		private void FormProcEdit_FormClosing(object sender,FormClosingEventArgs e) {
			...
			if(DialogResult==DialogResult.OK) {
				...
				Procedures.ComputeEstimates(ProcCur,PatCur.PatNum,ClaimProcsForProc,_isQuickAdd,PlanList,PatPlanList,BenefitList,PatCur.Age,SubList);
