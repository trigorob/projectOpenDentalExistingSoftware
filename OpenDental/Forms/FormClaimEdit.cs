...
///<summary></summary>
	public class FormClaimEdit : ODForm {
		...
		///<summary>The DefNum of the Custom Tracking status that is selected in comboCustomTracking.</summary>
		private long _selectedCustomTrackingDefNum;
    ...
    "arrier.";
			...
			this.textOrderingProviderOverride.TabIndex = 300;
			...
			this.butPickOrderProvReferral.TabIndex = 298;
			...
			this.butPickOrderProvInternal.TabIndex = 281;
			...
			this.butNoneOrderProv.TabIndex = 282;
			...
			this.gridStatusHistory.Location = new System.Drawing.Point(6, 28);
			this.gridStatusHistory.Name = "gridStatusHistory";
			this.gridStatusHistory.ScrollValue = 0;
			this.gridStatusHistory.Size = new System.Drawing.Size(946, 238);
			...
			// labelCustomTracking
			// 
			this.labelCustomTracking.Location = new System.Drawing.Point(8, 6);
			this.labelCustomTracking.Name = "labelCustomTracking";
			this.labelCustomTracking.Size = new System.Drawing.Size(126, 17);
			this.labelCustomTracking.TabIndex = 154;
			this.labelCustomTracking.Text = "Claim Custom Tracking";
			...
			this.comboCustomTracking.Location = new System.Drawing.Point(136, 4);
			this.comboCustomTracking.MaxDropDownItems = 100;
			this.comboCustomTracking.Name = "comboCustomTracking";
			this.comboCustomTracking.Size = new System.Drawing.Size(288, 21);
    ...
    "nt\" to finish this batch for daily reporting.";
		...
		private void FormClaimEdit_Load(object sender, System.EventArgs e) {
			...
			if(!IsNew && !Security.IsAuthorized(Permissions.ClaimDelete,ClaimCur.SecDateEntry)) {
      ...
      ClaimProcList=ClaimProcs.Refresh(PatCur.PatNum);
		...
		public void FillForm(){
			...
			if(DefC.Long[(int)DefCat.ClaimCustomTracking].Length==0){
				...
				_selectedCustomTrackingDefNum=ClaimCur.CustomTracking;
				FillCustomTrackingCombo();
			}
		...
		private void FillGrids(){
			...
			_listClaimProcsForClaim=ClaimProcs.RefreshForClaim(ClaimCur.ClaimNum);
			for(int i=0;i<_listClaimProcsForClaim.Count;i++){
				row=new ODGridRow();
				if(_listClaimProcsForClaim[i].LineNumber==0){
					row.Cells.Add("");
				}
				else{
					row.Cells.Add(_listClaimProcsForClaim[i].LineNumber.ToString());
				}
				string date=_listClaimProcsForClaim[i].ProcDate.ToShortDateString();
				if(_listClaimProcsForClaim[i].ProcNum==0) {//Total payment
					...
					date=_listClaimProcsForClaim[i].DateCP.ToShortDateString();
				}
				//Check if there was insurance payment amount entered but no claim payment (insurance check / not finalized). 
				if(_listClaimProcsForClaim[i].InsPayAmt > 0 && _listClaimProcsForClaim[i].ClaimPaymentNum==0) {
				...
				row.Cells.Add(date);
				row.Cells.Add(Providers.GetAbbr(((ClaimProc)_listClaimProcsForClaim[i]).ProvNum));
				if(_listClaimProcsForClaim[i].ProcNum==0) {
					...
					ProcCur=Procedures.GetProcFromList(ProcList,_listClaimProcsForClaim[i].ProcNum);
					row.Cells.Add(_listClaimProcsForClaim[i].CodeSent);
					if(!Clinics.IsMedicalPracticeOrClinic(Clinics.ClinicNum)) {
				...
				if(CultureInfo.CurrentCulture.Name.EndsWith("CA")) {//Canadian. en-CA or fr-CA
					...
					List<Procedure> labFeeProcs=Procedures.GetCanadianLabFees(_listClaimProcsForClaim[i].ProcNum,ProcList);
				...
				if(_listClaimProcsForClaim[i].ClaimPaymentTracking!=0) {
					row.Cells.Add(DefC.GetDef(DefCat.ClaimPaymentTracking,_listClaimProcsForClaim[i].ClaimPaymentTracking).ItemName);//EOB Code
				}
				else {
					row.Cells.Add("");
				}
				row.Cells.Add(_listClaimProcsForClaim[i].Remarks);
				claimFee+=_listClaimProcsForClaim[i].FeeBilled;
				dedApplied+=_listClaimProcsForClaim[i].DedApplied;
				insPayEst+=_listClaimProcsForClaim[i].InsPayEst;
				insPayAmt+=_listClaimProcsForClaim[i].InsPayAmt;
				//if(ClaimProcsForClaim[i].Status==ClaimProcStatus.Received
				//	|| ClaimProcsForClaim[i].Status==ClaimProcStatus.Supplemental) {
					writeOff+=_listClaimProcsForClaim[i].WriteOff;
				//}
		...
		private void FillStatusHistory() {
			...
			gridStatusHistory.Columns.Add(new ODGridColumn(Lan.g("TableStatusHistory","Description"),300));
			gridStatusHistory.Columns.Add(new ODGridColumn(Lan.g("TableStatusHistory","Log Note"),350));
			gridStatusHistory.Columns.Add(new ODGridColumn(Lan.g("TableStatusHistory","User"),100));
		...
		///<summary>Fills the custom tracking combo box and selects the definition that matches _selectedCustomTrackingDefNum.  Otherwise "None".</summary>
		private void FillCustomTrackingCombo() {
			...
			for(int i=0;i<DefC.Long[(int)DefCat.ClaimCustomTracking].Length;i++) {
				...
				if(_selectedCustomTrackingDefNum==DefC.Long[(int)DefCat.ClaimCustomTracking][i].DefNum) {
...
//fix: need to debug the recalculation feature to take this status into account.
		...
		private void butBatch_Click(object sender,EventArgs e) {
			...
			for(int i=0;i<_listClaimProcsForClaim.Count;i++) {
				if((_listClaimProcsForClaim[i].Status==ClaimProcStatus.Received
					...
					&& _listClaimProcsForClaim[i].InsPayAmt!=0) 
				{
					existsReceived=true;
				}
				...
				if(_listClaimProcsForClaim[i].Status==ClaimProcStatus.Supplemental && _listClaimProcsForClaim[i].InsPayAmt!=0 && _listClaimProcsForClaim[i].ClaimPaymentNum==0) {
					isSupplemental=true;
				}
			}
			if(!existsReceived) {
		...
		private void butAttachPerio_Click(object sender,EventArgs e) {
			...
			try {
				if(PrefC.AtoZfolderUsed==DataStorageType.DropboxAtoZ) {
					...
					FormP.TickMS=1000;
					DropboxApi.TaskStateUpload state=null;
					using(MemoryStream ms=new MemoryStream()) {
						bitmapBig.Save(ms,System.Drawing.Imaging.ImageFormat.Bmp);
						state=DropboxApi.UploadAsync(ProgramProperties.GetPropVal(Programs.GetProgramNum(ProgramName.Dropbox),DropboxApi.PropertyDescs.AccessToken)
							,ProgramProperties.GetPropVal(Programs.GetProgramNum(ProgramName.Dropbox),DropboxApi.PropertyDescs.AtoZPath)+"/EmailAttachments"
							,newName
							,ms.ToArray()
		...
		private void OpenFile() {
			...
			try {
				string tempFile
					=ODFileUtils.CombinePaths(PrefC.GetTempFolderPath(),ClaimCur.Attachments[listAttachments.SelectedIndex].DisplayedFileName);
				File.Copy(
      ...
      ClaimProcList=ClaimProcs.Refresh(PatCur.PatNum);
		...
		private void comboCustomTracking_SelectionChangeCommitted(object sender,EventArgs e) {
			if(comboCustomTracking.SelectedIndex<1) {//If the user is trying to set the Custom Tracking from a valid definition to "none"
				if(_selectedCustomTrackingDefNum!=0 
					&& !MsgBox.Show(this,MsgBoxButtons.OKCancel,"Setting the status to none will disable filtering in the Outstanding Claims Report."
						+"  Do you wish to set the status of this claim to none?")) 
				{
					//The user wants to undo their selection of None and put it back to what it was.
					FillCustomTrackingCombo();
				}
				else {
					//If the user really wants to set the Custom Tracking to "none" we will allow it.
					_selectedCustomTrackingDefNum=0;
				}
				return;
			}
			//The user selected a valid "custom tracking" status (even it if was the SAME status), ask them for a note.
			InputBox inputBox=new InputBox(Lan.g(this,"Please enter note below"));
			inputBox.setTitle(Lan.g(this,"Status History Note"));
			inputBox.ShowDialog();
			if(inputBox.DialogResult!=DialogResult.OK){
				//The user didn't make a new note, so to undo their selection and put it back to what it was.
				FillCustomTrackingCombo();
				return;
			}
			long defNum=DefC.Long[(int)DefCat.ClaimCustomTracking][comboCustomTracking.SelectedIndex-1].DefNum;
			_selectedCustomTrackingDefNum=defNum;
			//Update the old claim since current claim may have changed in the window. Current claim will be updated on OK Click
			_claimOld.CustomTracking=_selectedCustomTrackingDefNum;
			Claims.Update(_claimOld);
		...
		private void UpdateClaim(){
			...
			ClaimCur.CustomTracking=_selectedCustomTrackingDefNum;
