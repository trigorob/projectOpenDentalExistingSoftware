...
///<summary></summary>
	public class FormRecallList : ODForm {
		...
		private void butSchedPat_Click(object sender,EventArgs e) {
			...
			if(PatRestrictions.IsRestricted(SelectedPatNum,PatRestrict.ApptSchedule)) {
				return;
			}
		...
		private void butSchedFam_Click(object sender,EventArgs e) {
			...
			for(int i=0;i<fam.ListPats.Length;i++) {
				if(PatRestrictions.IsRestricted(fam.ListPats[i].PatNum,PatRestrict.ApptSchedule,true)) {
					patsRestricted++;
					continue;
				}
		...
		private void panelWebSched_MouseClick(object sender,MouseEventArgs e) {
			...
			//If the user didn't manually select any recalls we will automatically select all rows that have an email address.
			if(gridMain.SelectedIndices.Length==0) {
				ContactMethod cmeth;
				for(int i=0;i<table.Rows.Count;i++) {
			...
			if(gridMain.SelectedIndices.Length==0) {
				MsgBox.Show(this,"No patients prefer contact via email.");
				return;
			}
			//Now that there are rows guaranteed to be selected, check each row for an email and if their recall will yield available Web Sched time slots.
			...
			for(int i=gridMain.SelectedIndices.Length-1;i>=0;i--) {
				string email=table.Rows[gridMain.SelectedIndices[i]]["Email"].ToString();
				long recallNum=PIn.Long(table.Rows[gridMain.SelectedIndices[i]]["RecallNum"].ToString());
				DateTime dateDue=PIn.Date(table.Rows[gridMain.SelectedIndices[i]]["DateDue"].ToString());
				if(email.Trim()=="") {
					skippedEmail++;
			...
			if(skippedEmail > 0) {
				MessageBox.Show(Lan.g(this,"Selected patients skipped due to missing email addresses:")+" "+skippedEmail.ToString());
			}
			if(skippedTimeSlot > 0) {
				MessageBox.Show(Lan.g(this,"Selected patients skipped due to no available Web Sched time slots found:")+" "+skippedTimeSlot.ToString());
			}
			if(gridMain.SelectedIndices.Length==0) {
				MsgBox.Show(this,"No Web Sched emails sent.");
				return;
			}
			#endregion
