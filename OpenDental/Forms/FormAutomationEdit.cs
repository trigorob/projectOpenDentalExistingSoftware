...
namespace OpenDental{
	...
	public class FormAutomationEdit:ODForm {
		...
		private void FormAutomationEdit_Load(object sender, System.EventArgs e) {
			...
			AppointmentTypes.Listt.FindAll(x => !x.IsHidden || x.AppointmentTypeNum==AutoCur.AppointmentTypeNum).ForEach(x => _listAptTypes.Add(x));
		...
		private void comboAction_SelectedIndexChanged(object sender,EventArgs e) {
			...
			switch(_listAutoActions[comboAction.SelectedIndex]) {
				case AutomationAction.CreateCommlog:
					...
					DefC.Short[(int)DefCat.CommLogTypes].ToList().ForEach(x => comboActionObject.Items.Add(x.ItemName));
					comboActionObject.SelectedIndex=DefC.Short[(int)DefCat.CommLogTypes].ToList().FindIndex(x => x.DefNum==AutoCur.CommType);
				...
				case AutomationAction.ShowExamSheet:
					...
					comboActionObject.Visible=true;
					SheetDefC.Listt.ForEach(x => comboActionObject.Items.Add(x.Description));
					comboActionObject.SelectedIndex=SheetDefC.Listt.FindIndex(x => AutoCur.SheetDefNum==x.SheetDefNum);//can be -1
					return;
			}
		}
		...
		private void butOK_Click(object sender, System.EventArgs e) {
			...
			switch(AutoCur.AutoAction) {
				case AutomationAction.CreateCommlog:
					...
					AutoCur.CommType=DefC.Short[(int)DefCat.CommLogTypes][comboActionObject.SelectedIndex].DefNum;
				...
				case AutomationAction.ShowConsentForm:
					...
					if(SheetDefC.Listt[comboActionObject.SelectedIndex].SheetType!=dictAutoActionSheetType[AutoCur.AutoAction].Item1) {
						MessageBox.Show(this,Lan.g(this,"The selected sheet type must be")+" "+dictAutoActionSheetType[AutoCur.AutoAction].Item2+".");
						return;
					}
					AutoCur.SheetDefNum=SheetDefC.Listt[comboActionObject.SelectedIndex].SheetDefNum;
					break;
				case AutomationAction.SetApptStatus:
					if(comboActionObject.SelectedIndex==-1) {
