...
namespace OpenDental{
	...
	public class FormPerio : ODForm {
		...
		private void CreateNewPerioChart() {
			...
			if(PerioExams.ListExams.Count==0 || PrefC.GetBool(PrefName.PerioSkipMissingTeeth)) {
				for(int i=0;i<_listMissingTeeth.Count;i++) {
					...
					if(PrefC.GetBool(PrefName.PerioTreatImplantsAsNotMissing) && IsImplant(toothNum)) {
						listSkippedTeeth.Remove(toothNum);//Remove the tooth from the list of skipped teeth if it exists.
		...
		private bool IsImplant(int toothNum) {
			List<Procedure> listProcsForTooth=_listPatProcs.FindAll(x => x.ToothNum==toothNum.ToString());
