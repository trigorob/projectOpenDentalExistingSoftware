	...
	public partial class FormNewCropBilling:ODForm {
		...
		private void CreateChargeList() {
			...
			foreach(NewCropCharge charge in _listNewCropCharges) {
				charge.repeatCharge=GetRepeatChargeForProvider(charge);
			}
		}
		...
		private List<RepeatCharge> UpdateErxRepeatChargeFormat() {
			...
			if(listErxUpdateRepeatCharges.Count > 0) {
				foreach(NewCropCharge charge in _listNewCropCharges) {
					...
					repeatChargeForNpi.Note=npi+"  ErxAccountId="+charge.AccountId+comments;					
		...
		private RepeatCharge GetRepeatChargeForProvider(NewCropCharge charge) {
			...
			return listErxRepeatChargesForNpi.FirstOrDefault(x=>x.Note.Contains("ErxAccountId="+charge.AccountId));
		}
		...
		private void butProcess_Click(object sender,EventArgs e) {
			...
			foreach(NewCropCharge charge in _listNewCropCharges) {
				if(charge.repeatCharge==null) {//No such repeating charge exists yet for the given npi.
					...
					charge.repeatCharge.Note="NPI="+charge.NPI+"  ErxAccountId="+charge.AccountId;
