...
namespace OpenDental {
	public partial class FormPayConnect:ODForm {
		...
		private bool VerifyData(out int expYear,out int expMonth){
			...
			string paytype=ProgramProperties.GetPropVal(progCur.ProgramNum,"PaymentType",_clinicNum);
			if(ProgramProperties.GetPropVal(progCur.ProgramNum,"Username",_clinicNum)==""
				|| ProgramProperties.GetPropVal(progCur.ProgramNum,"Password",_clinicNum)=="" //if username or password is blank
		...
		private void butOK_Click(object sender,EventArgs e) {
			...
			if(_response==null || _response.Status.code!=0) {//error in transaction
				Cursor=Cursors.Default;
				if(!_isAddingCard) {//If adding the card, leave the window open so the user can try again.
			...
			if(sigBoxWrapper.GetSigChanged() && !string.IsNullOrEmpty(sigBoxWrapper.GetSignature(""))) {
				...
				sigResponse=Bridges.PayConnect.ProcessSignature(sigRequest,_clinicNum);
			}
