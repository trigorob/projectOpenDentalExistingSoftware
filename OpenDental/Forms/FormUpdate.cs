...
namespace OpenDental{
    ...
    "ersion only if you demand the latest features.  Be sure to update regularly.";
		...
		private void SetButtonVisibility() {
			...
			//construct the date time to use to compare.
			DateTime updateDateTime=new DateTime(DateTimeOD.Today.Year,DateTimeOD.Today.Month,DateTimeOD.Today.Day,updateTime.Hour,updateTime.Minute,0);
			...
			butDownloadMsiBuild.Enabled=updateDateTime < DateTime.Now;
			butDownloadMsiStable.Enabled=updateDateTime < DateTime.Now;
			butDownloadMsiBeta.Enabled=updateDateTime < DateTime.Now;
			butInstallBuild.Enabled=updateDateTime < DateTime.Now;
			butInstallStable.Enabled=updateDateTime < DateTime.Now;
			butInstallBeta.Enabled=updateDateTime < DateTime.Now;
		}
		...
		private void butInstallBuild_Click(object sender,EventArgs e) {
			...
			if(destDir==null) {//Not using A to Z folders?
				...
				if(PrefC.AtoZfolderUsed==DataStorageType.LocalAtoZ && !Directory.Exists(destPath2)) {
					...
					destDir=PrefC.GetTempFolderPath();//Dropbox needs it to be downloaded to a local temp folder
				}
		...
		private void butDownload_Click(object sender, System.EventArgs e){
			string patchName="Setup.exe";
			string destDir=ImageStore.GetPreferredAtoZpath();
			if(destDir==null || PrefC.AtoZfolderUsed==DataStorageType.DropboxAtoZ){
				destDir=PrefC.GetTempFolderPath();
			}
		...
		public static void DownloadInstallPatchFromURI(string downloadUri,string destinationPath,bool runSetupAfterDownload,bool showShutdownWindow,string destinationPath2){
			...
			if(PrefC.AtoZfolderUsed!=DataStorageType.DropboxAtoZ) {
				...
				FormP.DisplayText=Lan.g("FormUpdate","Uploading Setup File to Dropbox AtoZ...");//Upload unversioned setup file to AtoZ main folder.
				FormP.NumberFormat="F";
				...
				state=DropboxApi.UploadAsync(ProgramProperties.GetPropVal(Programs.GetProgramNum(ProgramName.Dropbox),DropboxApi.PropertyDescs.AccessToken)
					,ProgramProperties.GetPropVal(Programs.GetProgramNum(ProgramName.Dropbox),DropboxApi.PropertyDescs.AtoZPath)
				...
				if(destinationPath2!=null && destinationPath2!="") {//Upload a copy of the Setup.exe to a versioned setup file to SetupFiles folder.  Not always used.
					FormP=new FormProgress();
					FormP.DisplayText=Lan.g("FormUpdate","Uploading Setup File to Dropbox SetupFiles...");
					FormP.NumberFormat="F";
					FormP.NumberMultiplication=1;
					FormP.MaxVal=100;//Doesn't matter what this value is as long as it is greater than 0
					FormP.TickMS=1000;
					state=DropboxApi.UploadAsync(ProgramProperties.GetPropVal(Programs.GetProgramNum(ProgramName.Dropbox),DropboxApi.PropertyDescs.AccessToken)
						,ODFileUtils.CombinePaths(ProgramProperties.GetPropVal(Programs.GetProgramNum(ProgramName.Dropbox),DropboxApi.PropertyDescs.AtoZPath),"SetupFiles")
		...
		///<summary>No longer used here. Moved to FormAbout.cs.</summary>
