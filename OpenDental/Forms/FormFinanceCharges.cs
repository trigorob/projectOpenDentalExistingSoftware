...
///<summary></summary>
	public class FormFinanceCharges : ODForm {
		...
		private bool RunAgingEnterprise(bool isOnLoad=false) {
			...
			try {
				actionCloseProgress=ODProgress.ShowProgressStatus("FinanceCharge",Lan.g(this,"Calculating enterprise aging for all patients as of")+" "
		...
		private void butUndo_Click(object sender,EventArgs e) {
			...
			int rowsAffected=0;
			try {
				actionCloseProgress=ODProgress.ShowProgressStatus(chargeType+"Charge",Lan.g(this,"Deleting "+chargeType.ToLower()+" charge adjustments")+"...");
				Cursor=Cursors.WaitCursor;
			...
			if(PrefC.GetBool(PrefName.AgingIsEnterprise)) {
				...
				try {
					DateTime asOfDate=(PrefC.GetBool(PrefName.AgingCalculatedMonthlyInsteadOfDaily)?PrefC.GetDate(PrefName.DateLastAging):DateTime.Today);
					actionCloseProgress=ODProgress.ShowProgressStatus("FinanceCharge",Lan.g(this,"Calculating aging for all patients as of")+" "
						+asOfDate.ToShortDateString()+"...");
					Cursor=Cursors.WaitCursor;
					Ledgers.RunAging();
				}
		...
		private void butOK_Click(object sender,System.EventArgs e) {
			...
			Def[] billingTypesShort=DefC.Short[(int)DefCat.BillingTypes];
			List<long> listSelectedBillTypes=listBillType.SelectedIndices.OfType<int>().Select(x => billingTypesShort[x].DefNum).ToList();
			Action actionCloseProgress=null;
			int chargesAdded=0;
			try {
				actionCloseProgress=ODProgress.ShowProgressStatus("FinanceCharge",Lan.g(this,"Gathering patients with aged balances")+"...");
				List<PatAging> listPatAgings=Patients.GetAgingListSimple(listSelectedBillTypes);//Ordered by PatNum, for thread concurrency
				...
				foreach(PatAging patAgingCur in listPatAgings) {
					listActions.Add(new Action(() => {
						...
						else if(!AddFinanceCharge(patAgingCur.PatNum,date,textAPR.Text,textAtLeast.Text,textOver.Text,overallBalance,patAgingCur.PriProv)) {
							return;
						}
						chargesAdded++;
					}));
			...
			if(PrefC.GetBool(PrefName.AgingIsEnterprise)) {
				...
				DateTime asOfDate=(PrefC.GetBool(PrefName.AgingCalculatedMonthlyInsteadOfDaily)?PrefC.GetDate(PrefName.DateLastAging):DateTime.Today);
				actionCloseProgress=ODProgress.ShowProgressStatus("FinanceCharge",Lan.g(this,"Calculating aging for all patients as of")+" "+asOfDate.ToShortDateString()+"...");
				Cursor=Cursors.WaitCursor;
				try {
					Ledgers.RunAging();
				}
