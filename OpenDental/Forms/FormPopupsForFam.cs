		...
		private void FillGrid() {
			...
			foreach(long automationNum in listAutoNums) {
				...
				foreach(KeyValuePair<long,DateTime> kvp in dictPatDate) {
					...
					autoCur=Automations.Listt.FirstOrDefault(x => x.AutomationNum==automationNum);
					if(autoCur==null) {
						continue;
					}
					#region Dummy PopUp
			...
			gridMain.Rows.Clear();
			ODGridRow row;
			_listPopups.RemoveAll(x => x.PopupLevel==EnumPopupLevel.Automation && !Automations.Listt.Any(y => y.AutomationNum==x.PopupNum));
			for(int i=0;i<_listPopups.Count;i++) {
				...
				if(_listPopups[i].PopupLevel==EnumPopupLevel.Automation) {
					autoCur=Automations.Listt.FirstOrDefault(x => x.AutomationNum==_listPopups[i].PopupNum);//Get by PK.
