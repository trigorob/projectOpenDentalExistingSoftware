	...
	public class FormProviderSetup:ODForm {
		...
		private void FillGrid(bool needsRefresh=true) {
			...
			if(gridMain.SelectedIndices.Length==1){
				selectedProvNum=((Provider)gridMain.Rows[gridMain.SelectedIndices[0]].Tag).ProvNum;
			}
			...
			gridMain.Columns.Add(col);
			col=new ODGridColumn(Lan.g("TableProviderSetup","HideOnReports"),100,HorizontalAlignment.Center);
			gridMain.Columns.Add(col);
			if(!PrefC.GetBool(PrefName.EasyHideDentalSchools)) {
			...
			for(int i=0;i<_tableProvs.Rows.Count;i++) {
				...
				row=new ODGridRow();
				if(_tableProvs.Rows[i]["ProvStatus"].ToString()==POut.Int((int)ProviderStatus.Deleted)) {
				...
				if(_tableProvs.Rows[i]["IsHidden"].ToString()=="1"){
					row.Cells.Add("X");
				}
				else{
					row.Cells.Add("");
				}
				if(_tableProvs.Rows[i]["IsHiddenReport"].ToString()=="1") {
					row.Cells.Add("X");
				}
				else {
					row.Cells.Add("");
				...
				if(!PrefC.GetBool(PrefName.EasyHideDentalSchools)) {
					if(_tableProvs.Rows[i]["GradYear"].ToString()!=""){
						row.Cells.Add(_tableProvs.Rows[i]["GradYear"].ToString()+"-"+_tableProvs.Rows[i]["Descript"].ToString());
					}
					else{
						row.Cells.Add("");
					}
					if(_tableProvs.Rows[i]["IsInstructor"].ToString()=="1") {
						row.Cells.Add("X");
					}
					else {
						row.Cells.Add("");
				...
				row.Tag=_listProvs.Find(x => x.ProvNum==PIn.Long(_tableProvs.Rows[i]["ProvNum"].ToString()));
				gridMain.Rows.Add(row);
			}
			gridMain.EndUpdate();
			for(int i=0;i<gridMain.Rows.Count;i++){
				if(((Provider)gridMain.Rows[i].Tag).ProvNum==selectedProvNum) {
		...
		private void gridMain_CellDoubleClick(object sender,ODGridClickEventArgs e) {
			...
			if(!PrefC.GetBool(PrefName.EasyHideDentalSchools) && Providers.IsAttachedToUser(provSelected.ProvNum)) {//Dental schools is turned on and the provider selected is attached to a user.
				...
				if(!radioStudents.Checked) {
					FormProvEdit FormPE=new FormProvEdit();
					FormPE.ProvCur=(Provider)gridMain.Rows[gridMain.SelectedIndices[0]].Tag;
					FormPE.ShowDialog();
					if(FormPE.DialogResult!=DialogResult.OK) {
						return;
					}
					...
					FormProvStudentEdit FormPSE=new FormProvStudentEdit();
					FormPSE.ProvStudent=(Provider)gridMain.Rows[gridMain.SelectedIndices[0]].Tag;
					FormPSE.ShowDialog();
					if(FormPSE.DialogResult!=DialogResult.OK) {
						return;
					}
		...
		private void butMovePri_Click(object sender,EventArgs e) {
			...
			Provider provFrom=(Provider)gridMain.Rows[gridMain.SelectedIndices[0]].Tag;
			Provider provTo=_listProvs.Find(x => x.ProvNum==_provNumMoveTo);
		...
		private void butReassign_Click(object sender,EventArgs e) {
			...
			for(int i=0;i<gridMain.SelectedIndices.Length;i++) {
				provsFrom.Add(((Provider)gridMain.Rows[gridMain.SelectedIndices[0]].Tag).ProvNum);
			}
		...
		private void butAlphabetize_Click(object sender,EventArgs e) {
			...
			if(!MsgBox.Show(this,true,"Alphabetize all providers and move hidden providers to the bottom, followed by all non-person providers? This cannot be undone.")) {
				return;
			}
