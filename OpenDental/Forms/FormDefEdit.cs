...
///<summary></summary>
	public class FormDefEdit : ODForm {
		...
		private List<long> excludeSendNums;
		private List<long> excludeConfirmNums;
		...
		private void InitializeComponent()
		{
			...
			this.checkExcludeSend.Size = new System.Drawing.Size(154, 18);
			...
			this.checkExcludeConfirm.Size = new System.Drawing.Size(154, 18);
			this.checkExcludeConfirm.TabIndex = 8;
			this.checkExcludeConfirm.Text = "Exclude when confirming";
			// 
			// groupEConfirm
			// 
			this.groupEConfirm.Anchor = ((System.Windows.Forms.AnchorStyles)((System.Windows.Forms.AnchorStyles.Top | System.Windows.Forms.AnchorStyles.Right)));
			this.groupEConfirm.Controls.Add(this.checkExcludeSend);
			this.groupEConfirm.Controls.Add(this.checkExcludeConfirm);
			this.groupEConfirm.Location = new System.Drawing.Point(392, 61);
			this.groupEConfirm.Name = "groupEConfirm";
			this.groupEConfirm.Size = new System.Drawing.Size(163, 64);
		...
		private void FormDefEdit_Load(object sender, System.EventArgs e) {
			...
			if(DefCur.Category!=DefCat.ApptConfirmed) {
				...
				excludeSendNums=PrefC.GetString(PrefName.ApptConfirmExcludeESend).Split(',').ToList().Select(x => PIn.Long(x)).ToList();
				excludeConfirmNums=PrefC.GetString(PrefName.ApptConfirmExcludeEConfirm).Split(',').ToList().Select(x => PIn.Long(x)).ToList();
				checkExcludeSend.Checked=excludeSendNums.Contains(DefCur.DefNum);
				checkExcludeConfirm.Checked=excludeConfirmNums.Contains(DefCur.DefNum);
			}
		...
		private void butOK_Click(object sender, System.EventArgs e) {
			...
			if(DefCur.Category==DefCat.ApptConfirmed) {
				...
				if(checkExcludeSend.Checked) {
					excludeSendNums.Add(DefCur.DefNum);
				}
				else {
					excludeSendNums.RemoveAll(x => x==DefCur.DefNum);
				}
				string sendString=string.Join(",",excludeSendNums.Distinct().OrderBy(x => x));
				Prefs.UpdateString(PrefName.ApptConfirmExcludeESend,sendString);
				//==================== EXCLUDE CONFIRM ====================
				if(checkExcludeConfirm.Checked) {
					excludeConfirmNums.Add(DefCur.DefNum);
				}
				else {
					excludeConfirmNums.RemoveAll(x => x==DefCur.DefNum);
				}
				string confirmString = string.Join(",",excludeConfirmNums.Distinct().OrderBy(x => x));
