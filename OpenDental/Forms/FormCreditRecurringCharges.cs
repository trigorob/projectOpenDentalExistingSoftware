...
namespace OpenDental {
	public partial class FormCreditRecurringCharges:ODForm {
		...
		private void FillGrid(bool isFromDb=false) {
			...
			gridMain.Columns.Add(new ODGridColumn(Lan.g("TableRecurring","Name"),PrefC.HasClinicsEnabled?220:255));
			if(PrefC.HasClinicsEnabled) {
				gridMain.Columns.Add(new ODGridColumn(Lan.g("TableRecurring","Clinic"),75));
			}
			gridMain.Columns.Add(new ODGridColumn(Lan.g("TableRecurring","Family Bal"),PrefC.HasClinicsEnabled?75:90,HorizontalAlignment.Right));
			gridMain.Columns.Add(new ODGridColumn(Lan.g("TableRecurring","PayPlan Due"),PrefC.HasClinicsEnabled?85:90,HorizontalAlignment.Right));
			gridMain.Columns.Add(new ODGridColumn(Lan.g("TableRecurring","Total Due"),PrefC.HasClinicsEnabled?70:90,HorizontalAlignment.Right));
			gridMain.Columns.Add(new ODGridColumn(Lan.g("TableRecurring","Repeat Amt"),PrefC.HasClinicsEnabled?85:100,HorizontalAlignment.Right));//RptChrgAmt
		...
		private void FillDataTableHelper() {
			...
			for(int i=_tableAllRCs.Rows.Count-1;i>-1;i--) {//loop through backwards since we may remove rows if the charge amount is <=0 or patCur==null
				...
				double famBalTotal=PIn.Double(rowCur["FamBalTotal"].ToString());
				double rptChargeAmt;
				//will be 0 if this is not a payplan row, if negative don't subtract from the FamBalTotal
				double payPlanDue=Math.Max(PIn.Double(rowCur["PayPlanDue"].ToString()),0);
				...
				if(PrefC.IsODHQ) {//HQ calculates repeating charges based on the presence of procedures on the patient's account that are linked to the CC
					if(PrefC.GetBool(PrefName.BillingUseBillingCycleDay)) {
						rptChargeAmt=CreditCards.TotalRecurringCharges(patNum,procedures,PIn.Int(rowCur["BillingCycleDay"].ToString()));
					}
					else {
						rptChargeAmt=CreditCards.TotalRecurringCharges(patNum,procedures,PIn.Date(rowCur["DateStart"].ToString()).Day);
					}
					...
					rptChargeAmt=PIn.Double(rowCur["ChargeAmt"].ToString());
				}
		...
		private void SendPayConnect() {
			...
			if(PrefC.AtoZfolderUsed==DataStorageType.LocalAtoZ) {
				...
				FormP.TickMS=1000;
				DropboxApi.TaskStateUpload state=DropboxApi.UploadAsync(ProgramProperties.GetPropVal(Programs.GetProgramNum(ProgramName.Dropbox),DropboxApi.PropertyDescs.AccessToken)
					,ProgramProperties.GetPropVal(Programs.GetProgramNum(ProgramName.Dropbox),DropboxApi.PropertyDescs.AtoZPath)+"/PayConnect"
		...
		private void CreatePayment(Patient patCur,int selectedIndex,string note) {
			...
			paymentCur.PayAmt=PIn.Double(rowCur["ChargeAmt"].ToString());
