		...
		private void butMerge_Click(object sender,EventArgs e) {
			...
			if(_patFrom.PatNum==_patFrom.Guarantor) {
				...
				if(fam.ListPats.Length>1) {
					msgText=Lan.g(this,"The patient you have chosen to merge from is a guarantor.  Merging this patient into another account will cause all "
						+"familiy members of the patient being merged from to be moved into the same family as the patient account being merged into.")+"\r\n"
			...
			if(Patients.MergeTwoPatients(_patTo.PatNum,_patFrom.PatNum)) {
				...
				if(PrefC.AtoZfolderUsed==DataStorageType.LocalAtoZ) {
					...
					FormP.TickMS=1000;
					DropboxApi.TaskStateMove state=DropboxApi.MoveAsync(ProgramProperties.GetPropVal(Programs.GetProgramNum(ProgramName.Dropbox),DropboxApi.PropertyDescs.AccessToken)
						,atozFrom
						,atozTo
						,new DropboxApi.ProgressHandler(FormP.OnProgress));
					if(FormP.ShowDialog()==DialogResult.Cancel) {
						state.DoCancel=true;
						fileCopyFailures=state.CountMoveTotal-state.CountMoveSuccess;
					}
					else {
						fileCopyFailures=state.CountMoveFailed;
					}
				}
