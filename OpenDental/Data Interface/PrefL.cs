	...
	public class PrefL{
		...
		public static bool CheckProgramVersion(bool isSilent) {
				...
				&& !ODEnvironment.IdIsThisComputer(PrefC.GetString(PrefName.WebServiceServerName).ToLower()))
			{
				...
				//The logic for downgrading client version is broken.  We're temporarily removing this option until it is restored so users are not connected
				// with the future version to an older database/server version.
			...
			//At this point we know 100% it's going to be an upgrade
				...
				ServicesHelper.TryInstallOpenDentalService(isSilent);
