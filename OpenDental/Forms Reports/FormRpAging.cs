...
namespace OpenDental{
	...
	public class FormRpAging : ODForm {
		...
		private void FormAging_Load(object sender, System.EventArgs e) {
			...
			if(PrefC.GetBool(PrefName.EasyNoClinics)) {
				...
				if(!Security.CurUser.ClinicIsRestricted) {
					_listClinics.Add(new Clinic() { Abbr=Lan.g(this,"Unassigned") });//Seed with "Unassigned"
				}
				Clinics.GetForUserod(Security.CurUser).ForEach(x => _listClinics.Add(x));
				_listClinics.ForEach(x => listClin.Items.Add(x.Abbr));
				if(Clinics.ClinicNum==0) {
					checkAllClin.Checked=true;//event handler will select all indices
				}
				else {
					listClin.SetSelected(_listClinics.FindIndex(x => x.ClinicNum==Clinics.ClinicNum),true);
				}
			}
		}
		...
		private void checkAllClin_Click(object sender,EventArgs e) {
			if(!checkAllClin.Checked) {//strictly if user clicks to uncheck the box
		...
		private void checkAllClin_CheckedChanged(object sender,EventArgs e) {
			if(checkAllClin.Checked) {
				for(int i=0;i<listClin.Items.Count;i++) {//checked by code or by user clicking
		...
		private void butOK_Click(object sender, System.EventArgs e) {
			...
			if(PrefC.GetBool(PrefName.ReportsShowPatNum)){
				cmd+=DbHelper.Concat("guar.PatNum","' - '","guar.LName","', '","guar.FName","' '","guar.MiddleI");
			}
			else{
				cmd+=DbHelper.Concat("guar.LName","', '","guar.FName","' '","guar.MiddleI");
			}
			cmd+=",guarAging.Bal_0_30,guarAging.Bal_31_60,guarAging.Bal_61_90,guarAging.BalOver90,guarAging.BalTotal,guarAging.InsEst,"
				+"guarAging.BalTotal-guarAging.InsEst AS ";
			...
			cmd+="FROM ("+Ledgers.GetAgingQueryString(0,asOfDate)+") guarAging "
				+"INNER JOIN patient guar ON guar.PatNum=guarAging.PatNum "
				+"WHERE ";
			if(checkOnlyNeg.Checked){
				cmd+="guarAging.BalTotal < -0.005 ";
			}
			else{
				cmd+="(";
				if(radioAny.Checked){
					cmd+="guarAging.Bal_0_30 > 0.005 OR guarAging.Bal_31_60 > 0.005 OR guarAging.Bal_61_90 > 0.005 OR guarAging.BalOver90 > 0.005";
				}
				else if(radio30.Checked){
					cmd+="guarAging.Bal_31_60 > 0.005 OR guarAging.Bal_61_90 > 0.005 OR guarAging.BalOver90 > 0.005";
				}
				else if(radio60.Checked){
					cmd+="guarAging.Bal_61_90 > 0.005 OR guarAging.BalOver90 > 0.005";
				}
				else if(radio90.Checked){
					cmd+="guarAging.BalOver90 > 0.005";
				}
				if(checkIncludeNeg.Checked){
					cmd+=" OR guarAging.BalTotal < -0.005";
				}
				cmd+=") ";
			}
			if(checkExcludeInactive.Checked) {
				cmd+="AND guar.Patstatus != 2 ";
			}
			if(checkBadAddress.Checked) {
				cmd+="AND guar.Zip != '' ";
			}
			if(!checkBillTypesAll.Checked){//validated to have at least one BillingType selected above
				cmd+="AND guar.BillingType IN ("
					+string.Join(",",listBillType.SelectedIndices.OfType<int>().Select(x => POut.Long(DefC.Short[(int)DefCat.BillingTypes][x].DefNum)))+") ";
			}
			if(!checkProvAll.Checked) {//validated to have at least one prov selected above
				cmd+="AND guar.PriProv IN ("
					+string.Join(",",listProv.SelectedIndices.OfType<int>().Select(x => POut.Long(_listProviders[x].ProvNum)))+") ";
			}
			if(PrefC.HasClinicsEnabled) {//validated to have at least one clinic selected if clinics are enabled above
				...
				cmd+="AND guar.ClinicNum IN ("+string.Join(",",listClin.SelectedIndices.OfType<int>().Select(x => POut.Long(_listClinics[x].ClinicNum)))+") ";
			}
			cmd+="ORDER BY guar.LName,guar.FName";
			report.Query=cmd;
