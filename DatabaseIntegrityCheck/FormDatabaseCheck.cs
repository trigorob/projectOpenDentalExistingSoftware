...
namespace DatabaseIntegrityCheck {
	public partial class FormDatabaseCheck:Form {
		...
		private void butRun_Click(object sender,EventArgs e) {
			...
			cmd.CommandTimeout=1200;//Timeout for all check table and repair table commands.  Twenty minutes per command.  Needed for large databases (NDAG)
			...
			try {
				...
				for(int i=0;i<tableName.Length;i++) {
					ShowProgress("Checking table "+tableName[i]);
					try {
						...
						listCheckFailed.Add(tableName[i]);
					}
				}
				ShowProgress("");
				...
				Cursor=Cursors.WaitCursor;
				for(int i=0;i<corruptTables.Count;i++) {
					ShowProgress("Repairing table "+corruptTables[i]);
