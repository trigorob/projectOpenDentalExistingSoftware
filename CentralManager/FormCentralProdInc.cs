...
namespace CentralManager {
	public partial class FormCentralProdInc:Form {
		...
		private void ConnectAndRunAnnualReport(ODThread odThread) {
			...
			listProdData=RpProdInc.GetAnnualData(_dateFrom,_dateTo,listProvs,listClinics,radioWriteoffPay.Checked,true,true,checkShowUnearned.Checked);
			odThread.Tag=new object[] { listProdData,conn };
		}
		...
		private void ConnectAndRunMonthlyReport(ODThread odThread) {
			...
			listClinics.Add(unassigned);
			listProdData=RpProdInc.GetMonthlyData(_dateFrom,_dateTo,listProvs,listClinics,radioWriteoffPay.Checked,true,true,checkShowUnearned.Checked);
			odThread.Tag=new object[] { listProdData,conn };
		}
		...
		private void ConnectAndRunDailyReport(ODThread odThread) {
			...
			unassigned.Description="Unassigned";//Same issue here as above.
			listClinics.Add(unassigned);
			dataSetProdData=RpProdInc.GetDailyData(_dateFrom,_dateTo,listProvs,listClinics,radioWriteoffPay.Checked,true,true,false,true,checkShowUnearned.Checked);
			odThread.Tag=new object[] { dataSetProdData,conn };
		}
		...
		private void ConnectAndRunProviderReport(ODThread odThread) {
			...
			unassigned.Description="Unassigned";//Same issue here as above.
			listClinics.Add(unassigned);
			listProdData=RpProdInc.GetProviderDataForClinics(_dateFrom,_dateTo,listProvs,listClinics,radioWriteoffPay.Checked,true,true,checkShowUnearned.Checked);
